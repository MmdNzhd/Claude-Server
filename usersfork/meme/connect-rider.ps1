...
if ($go) {
    ...
    try {
        :sessionLoop while ($true) {
            ...
            $riderPath = Get-Command rider64.exe -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Source
            if (-not $riderPath) {
                ...
                foreach ($pattern in $possiblePaths) {
                    ...
                    if ($found) { $riderPath = $found.FullName; break }
                }
            }
            
            if ($riderPath) {
                # Use JetBrains Gateway protocol for remote SSH
                # Format: jetbrains-gateway://connect#host=hostname&user=username&projectPath=/path
                $encodedPath = [System.Web.HttpUtility]::UrlEncode($go.Path)
                $gatewayUri = "jetbrains-gateway://connect#host=${Alias}&user=${script:LaptopUser}&projectPath=${encodedPath}&type=ssh&deploy=false&newUi=true"
                Start-Process $gatewayUri
                StepOk $($go.Path)
            } else {
                StepFail "Rider not found"
                Warn "Install JetBrains Rider or add rider64.exe to PATH"
                Warn "You can manually connect via JetBrains Gateway -> SSH"
                Write-Host "      SSH Host: $Alias" -ForegroundColor DarkGray
                Write-Host "      User: ${script:LaptopUser}" -ForegroundColor DarkGray
                Write-Host "      Path: $($go.Path)" -ForegroundColor DarkGray
            }
