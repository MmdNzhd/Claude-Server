...
Current connect scripts: v20260802.1
...
5. Every run after: type a project number (empty Enter does nothing),
   ...
   Header must show:
     claude-server  |  192.168.210.240  |  v20260802.1
...
If you still see a lasting "Claude Connect" cmd on the taskbar:
  1. Confirm header version is current (v20260802.1+).
...
--------------------------
  Only ONE project is mounted per session. Requires connect scripts v20260802.1+
...
If connect.bat says OUTDATED:
  - Missing connect-ui.ps1 or version is not v20260802.1
...
LOGS (durable on laptop + synced to server, v20260802.1+)
------------------------------------------------------------
