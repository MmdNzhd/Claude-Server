...
$Alias    = "claude-server"
$script:ConnectVersion = '20260721.49'
...
$script:ConnectBuildId = 'e937c70c-8289-47fa-9d1c-46f637abde5d'
...
if (Get-Command Request-ConnectLogSync -ErrorAction SilentlyContinue) { Request-ConnectLogSync | Out-Null }
...
if (Get-Command Write-ConnectSessionContext -ErrorAction SilentlyContinue) { Write-ConnectSessionContext -Phase 'server_ready' }
            ...
            if ($authRelaunch -or (-not $editorOpened -and -not $onCorrectFolder)) {
                ...
                if ($authRelaunch -or -not $editorOpened) {
                    ...
                    if (-not $launchOk) {
                        StepFail "$EditorName failed to start (elevated launch). Keep connect open and press O, or launch Cursor manually with ClaudeServerCursorProfile."
            ...
            while ($true) {
                ...
                if ([Console]::KeyAvailable) {
                    ...
                    # VK fallback ONLY for null/control KeyChar - never for Persian/other printable non-ASCII (Ø¶ on Q).
