...
function Get-SocksProxyPort {
    # Local-only listener port for the -L forward piggybacked on the same reverse-tunnel ssh
    # process. Never leaves 127.0.0.1 on the laptop, so it only needs to be unique across
    # concurrent sessions on THIS machine - reusing the existing TunnelSlot (0-9, already
    # collision-free per laptop) avoids inventing a second allocation scheme.
    $slot = 0
    if ($null -ne $script:TunnelSlot) { $slot = [int]$script:TunnelSlot }
    return 19080 + $slot
}

function Get-HttpProxyPort {
    # Local HTTP forward for Cursor MCP (undici rejects socks5 in http.proxy).
    $slot = 0
    if ($null -ne $script:TunnelSlot) { $slot = [int]$script:TunnelSlot }
    return 19180 + $slot
}
...
function Set-SocksProxyPortOnReuse {
    ...
    # and remote xray all check out. Otherwise null (proxy skipped until next full respawn).
    param(
    ...
    try {
        ...
        if (-not $localOk) { return }
        $httpLocalOk = $false
        try {
            $httpLocalOk = [bool](Test-NetConnection -ComputerName 127.0.0.1 -Port $httpCandidate -WarningAction SilentlyContinue -InformationLevel Quiet -ErrorAction SilentlyContinue)
        } catch { $httpLocalOk = $false }
        if (-not $httpLocalOk) { return }
...
function Ensure-SessionTunnel {
    ...
    $script:SocksProxyPort = $null
    $script:HttpProxyPort = $null
    # ExitOnForwardFailure=yes only fails on local BIND, not remote target reachability -
    # probe server xray SOCKS before adding -L. A bad local bind would kill the WHOLE tunnel,
    # including the critical -R mount forward, so skip the proxy leg when port is busy.
    $sshArgs = @(
