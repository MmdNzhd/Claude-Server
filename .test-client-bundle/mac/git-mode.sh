...
local_port_listening() {
    # Return 0 if something accepts on 127.0.0.1:$1 (forward active).
    local port="$1"
    if command -v nc >/dev/null 2>&1; then
        nc -z 127.0.0.1 "$port" 2>/dev/null
        return $?
    fi
    python3 -c "import socket,sys; s=socket.socket(); s.settimeout(1); r=s.connect_ex(('127.0.0.1',int(sys.argv[1]))); s.close(); sys.exit(0 if r==0 else 1)" "$port" 2>/dev/null
}
...
set_socks_proxy_port_on_reuse() {
    ...
    # and remote xray all check out. Otherwise empty (proxy skipped until next full respawn).
    local tunnel_pid="${1:-${bg_pid:-}}"
    SOCKS_PROXY_PORT=""
    HTTP_PROXY_PORT=""
...
ensure_session_tunnel() {
    ...
    if ! remote_xray_socks_open; then
        ...
        if local_port_free "$socks_candidate"; then
            ...
            SOCKS_PROXY_PORT="$socks_candidate"
            if declare -F connect_log >/dev/null 2>&1; then
                connect_log "ENSURE_TUNNEL proxy_leg=-L local=$socks_candidate remote=${XRAY_SERVER_SOCKS_PORT} after_legacy_cleanup" 'INFO'
            append_http_proxy_leg
            fi
        ...
        SOCKS_PROXY_PORT="$socks_candidate"
        if declare -F connect_log >/dev/null 2>&1; then
            connect_log "ENSURE_TUNNEL proxy_leg=-L local=$socks_candidate remote=${XRAY_SERVER_SOCKS_PORT}" 'INFO'
        append_http_proxy_leg
        fi
    fi
