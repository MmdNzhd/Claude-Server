...
$Alias    = "claude-server"
$script:ConnectVersion = '20260720.13'
    ...
    if ($dir) {
        ...
        if (Test-Path $mountSrc) {
            ...
            if (-not ($localHash -and $remoteHash -and ($localHash.ToLower() -eq $remoteHash.ToLower()))) {
                $proc = Start-Process -FilePath 'scp' -PassThru -NoNewWindow `
                    -ArgumentList @('-o', 'BatchMode=yes', '-o', 'ConnectTimeout=30', '-q', $uploadSrc, "${Alias}:~/.local/bin/claude-mount.new")
                $scpJobs += @{ Name = 'claude-mount'; Proc = $proc }
            }
        }
        ...
        if (Test-Path $gitSrc) {
            ...
            if (-not ($gitLocal -and $gitRemote -and ($gitLocal.ToLower() -eq $gitRemote.ToLower()))) {
                $proc = Start-Process -FilePath 'scp' -PassThru -NoNewWindow `
                    -ArgumentList @('-o', 'BatchMode=yes', '-o', 'ConnectTimeout=30', '-q', $gitSrc, "${Alias}:~/.local/bin/claude-git-setup.new")
                $scpJobs += @{ Name = 'claude-git-setup'; Proc = $proc }
            }
        }
    }
...
"@ | Add-Content -Path $SshCfgPath -Encoding ASCII
    ...
    foreach ($job in $scpJobs) {
        try {
            Wait-Process -Id $job.Proc.Id -ErrorAction Stop
...
function Invoke-SshXCore {
    ...
    $lines = @(& ssh @sshArgs $Alias $wrapped 2>&1)
    $sw.Stop()
...
if (Get-Command Request-ConnectLogSync -ErrorAction SilentlyContinue) { Request-ConnectLogSync | Out-Null } elseif (Get-Command Sync-ConnectLogToServer -ErrorAction SilentlyContinue) { Sync-ConnectLogToServer | Out-Null }  # ship BOOTSTRAP/UPDATE + session so far
...
if (Get-Command Sync-ConnectLogToServer -ErrorAction SilentlyContinue) { Sync-ConnectLogToServer -Force | Out-Null }  # foreign-session guard: flush before the security decision
...
Write-Host ''
if (Get-Command Request-ConnectLogSync -ErrorAction SilentlyContinue) { Request-ConnectLogSync | Out-Null } elseif (Get-Command Sync-ConnectLogToServer -ErrorAction SilentlyContinue) { Sync-ConnectLogToServer | Out-Null }
