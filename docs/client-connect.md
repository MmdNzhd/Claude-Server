...
**Current client version:** **`20260715.16`**
...
If Cursor opens **Agent home** instead of the project folder, check `connect.log` beside `connect.bat`. v20260715.16+ uses `--new-window` when not on the correct `folder-uri`.
...
If Cursor opens **Agent home** instead of the project folder, press **`O`** in the connect menu or reconnect with v20260715.16+.
...
### Performance marks (v20260715.16+)
...
- Server mount fix: `sudo claude-server deploy-mount-fix` (requires connect.bat v20260715.16+)
...
| Cursor Agent home, not project | Update to v20260715.16+, check `connect.log`, press `O` |
| Cursor Chat cannot send (Mac/Win) | Reconnect, then **Developer �+' Reload Window** in `[Claude Server]` window |
| Mac Remote SSH `listen EINVAL` | Update to v20260715.16+; or `launchctl setenv TMPDIR /tmp` + quit Cursor fully |
