...
### Project menu — no Enter default

Type a **project number** to connect. **Empty Enter does nothing** (does not auto-select the last project).

Each OS sees **only its own projects** in the menu (Mac: `/Users/...` paths; Windows: `D:/...` paths). Other-OS projects stay on the server but are hidden — the header shows e.g. `1 for this Mac (13 Windows-only hidden)`.
...
### Mac reverse tunnel — ports, slots, multi-laptop
...
| … | … | Up to 10 machines per server account |
...
1. `acquire_tunnel_port` — scan slots; skip ports whose SSH banner is another OS (Windows/Linux on a Mac session).
2. `sanitize_ssh_alias_config` — remove legacy `RemoteForward` from `~/.ssh/config` (must **not** use `ClearAllForwardings` on the tunnel command — OpenSSH clears `-R` too).
3. `tunnel_up` — TCP open **and** banner matches `LAPTOP_OS`.
4. `verify_laptop_reverse_ssh` — server key `claude_laptop` → laptop via tunnel (same path as sshfs).
...
**Do not** put `RemoteForward` in `Host claude-server` — tunnel is started explicitly with `ssh -N -R $PORT:localhost:22`.
...
| Mac mount: connection reset | Windows on slot 0 — Mac uses slot 1+; run `claude-mount tunnel-status` on server |
