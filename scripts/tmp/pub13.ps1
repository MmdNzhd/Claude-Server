...
foreach ($t in @(
  ...
  Write-Host ("SYNC {0} ver={1} fast={2} tcp1={3}" -f $t, (Get-Content (Join-Path $t 'connect-version.txt') -Raw).Trim(), [int]($raw -match 'ACQUIRE_FAST claim_free'), [int]($raw -match 'Closed port cannot be a live foreign'))
}
