...
$Alias    = "claude-server"
$script:ConnectVersion = '20260719.31'
...
function Die($m) {
    ...
    Write-Host ""; Write-Host "  [X] $m" -ForegroundColor Red; Wait-ConnectExit -Reason 'require_fail' -Code 1
}
function Warn($m) {
...
function Initialize-ServerSession {
    ...
    if ($dir) {
        if (Get-Command Invoke-SshXChecked -ErrorAction SilentlyContinue) { [void](Invoke-SshXChecked -RemoteCmd 'mkdir -p ~/.local/bin' -Label 'MKDIR_LOCAL_BIN') } else { $null = SshX 'mkdir -p ~/.local/bin' 2>$null; if ($global:LASTEXITCODE -ne 0) { if (Get-Command Write-ConnectLog -EA SilentlyContinue) { Write-ConnectLog "MKDIR_LOCAL_BIN fail exit=$($global:LASTEXITCODE)" 'WARN' } } }
...
"@ | Add-Content -Path $SshCfgPath -Encoding ASCII
    ...
    if ($dir -and $scpProcs.Count -gt 0) {
        ...
        if ($chmodCmd.Count -gt 0) { if (Get-Command Invoke-SshXChecked -ErrorAction SilentlyContinue) { [void](Invoke-SshXChecked -RemoteCmd ($chmodCmd -join '; ') -Label 'PUSH_CHMOD_CONNECT') } else { $null = SshX ($chmodCmd -join '; ') 2>$null; if ($global:LASTEXITCODE -ne 0) { if (Get-Command Write-ConnectLog -EA SilentlyContinue) { Write-ConnectLog "PUSH_CHMOD_CONNECT fail exit=$($global:LASTEXITCODE)" 'WARN' } } } }
    }
...
if (Get-Command Write-ConnectSessionContext -ErrorAction SilentlyContinue) { Write-ConnectSessionContext -Phase 'project_selected' }
    ...
    if (Get-Command Write-ConnectSessionContext -ErrorAction SilentlyContinue) { Write-ConnectSessionContext -Phase 'session_loop' }
            }
            $tunnelReused = $false
...
if (Get-Command Write-ConnectSessionContext -ErrorAction SilentlyContinue) { Write-ConnectSessionContext -Phase 'server_ready' }
            ...
            while ($true) {
                ...
                if ($EditorCmd -eq 'cursor' -and ((Get-Date) - $lastEditorCheckAt -gt [TimeSpan]::FromSeconds(2))) {
                    ...
                    $editorLabel = if ($onFolderNow) { $EditorName } elseif ($script:EditorSeenOpen) { 'sticky' } elseif ($windowOpen) { 'agent' } else { 'closed' }
