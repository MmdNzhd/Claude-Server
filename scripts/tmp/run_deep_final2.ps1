...
Write-Host '======== VERSIONS (timeout 8s) ========'
$smart = & ssh -o BatchMode=yes -o ConnectTimeout=5 -o ServerAliveInterval=2 -o ServerAliveCountMax=2 smart@192.168.210.240 'cat /usr/local/share/claude-client/connect-version.txt' 2>$null
if ($LASTEXITCODE -eq 0 -and $smart) {
  $smart = ($smart | Out-String).Trim()
  if ($smart -eq '20260717.22') { OK "smart=$smart frozen" } else { FAIL "smart=$smart expected .22" }
} else { FAIL 'smart unreachable' }

$sepidz = & ssh -o BatchMode=yes -o ConnectTimeout=5 -o ServerAliveInterval=2 -o ServerAliveCountMax=2 sepidz@192.168.250.70 'cat /usr/local/share/claude-client/connect-version.txt' 2>$null
if ($LASTEXITCODE -eq 0 -and $sepidz) {
  $sepidz = ($sepidz | Out-String).Trim()
  if ($sepidz -eq '20260717.33') { OK "sepidz=$sepidz" } else { FAIL "sepidz=$sepidz expected .33" }
} else { FAIL 'sepidz unreachable' }
...
[IO.File]::WriteAllText("$env:TEMP\rdf2.sh", $wrap)
scp -o BatchMode=yes -q "$env:TEMP\rdf2.sh" 'sepidz@192.168.250.70:/tmp/rdf2.sh'
ssh -o BatchMode=yes -o ConnectTimeout=20 sepidz@192.168.250.70 'bash /tmp/rdf2.sh'
