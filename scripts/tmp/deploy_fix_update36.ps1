...
foreach ($name in @('claude-code-sepidz-20260717','claude-code-sepidz-20260718')) {
  $bad = Join-Path $env:USERPROFILE "Desktop\claude-publish\$name\claude-code\windows"
  if (Test-Path $bad) {
    Copy-Item (Join-Path $clientRoot 'windows\*') $bad -Force -Recurse
    Write-Host ("FIXED {0} ver={1} ip={2}" -f $name, (Get-Content (Join-Path $bad 'connect-version.txt') -Raw).Trim(), [regex]::Match((Get-Content (Join-Path $bad 'connect.ps1') -Raw),'192\.168\.\d+\.\d+').Value)
  }
}
...
Write-Host '=== SIMULATE AUTO-UPDATE (.8 -> .36 from Sepidz) ==='
...
if (-not $p2.WaitForExit(180000)) { try{$p2.Kill()}catch{}; Write-Host (Get-Content $log -Raw); throw 'SIM TIMEOUT' }
...
$p3=Start-Process ssh -ArgumentList @('-o','BatchMode=yes','-o','ControlMaster=no','-o','ConnectTimeout=8','smart@192.168.210.240','cat /usr/local/share/claude-client/connect-version.txt') -NoNewWindow -PassThru -RedirectStandardOutput $smartOut -RedirectStandardError "$smartOut.err"
[void]$p3.WaitForExit(12000)
...
if ($smart -ne '20260717.22') { Write-Host "WARN smart=$smart (expected .22)" }
