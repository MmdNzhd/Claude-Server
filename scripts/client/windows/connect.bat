...
if exist "%HERE%connect-heal.ps1" (
    ...
    powershell -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -Command "try { $here='%HERE_NOTRAIL%'; $canon=Join-Path $env:USERPROFILE 'Desktop\Claude-Connect'; $stable=Join-Path $env:USERPROFILE 'Desktop\claude-publish\claude-code-client\windows'; $legacy=($here -match 'claude-code-client-\d{8}' -or $here -match 'claude-code-sepidz-\d{8}' -or $here -match '[\\/]claude-publish[\\/]'); function Good($d){ if(-not (Test-Path (Join-Path $d 'connect.bat'))){return $false}; if(-not (Test-Path (Join-Path $d 'cursor-proxy-sidecar.ps1'))){return $false}; $u=Join-Path $d 'connect-update.ps1'; if(-not (Test-Path $u)){return $false}; $r=Get-Content -LiteralPath $u -Raw -EA SilentlyContinue; if($r -match 'UpdateEndpointTarget'){return $false}; return $true }; $src=$null; foreach($c in @($canon,$stable)){ if(Good $c){ $src=$c; break } }; if(-not $src){ exit 0 }; $names=@('connect.bat','connect-boot.ps1','connect-preflight.ps1','connect-bootstrap.ps1','connect-heal.ps1','connect.ps1','connect-update.ps1','cursor-proxy-sidecar.ps1','connect-ui.ps1','connect-diagnostic.ps1','editor-launch.ps1','git-mode.ps1','cursor-auth-laptop.ps1','windows-mcp-laptop.ps1','connect-version.txt'); if(-not (Good $canon)){ New-Item -ItemType Directory -Force -Path $canon|Out-Null; foreach($n in $names){ $s=Join-Path $src $n; if(Test-Path $s){ Copy-Item -Force $s (Join-Path $canon $n) } } }; $need=(-not (Good $here)); if($need){ foreach($n in $names){ $s=Join-Path $canon $n; if(Test-Path $s){ Copy-Item -Force $s (Join-Path $here $n) } } }; if($legacy -and (Good $canon) -and ($here -ne $canon)){ Set-Content -LiteralPath (Join-Path $env:TEMP 'claude-connect-relaunch.dir') -Value $canon -Encoding ASCII; exit 2 }; exit 0 } catch { exit 1 }"
    set "HEAL_EC=!errorlevel!"
)
if "!HEAL_EC!"=="2" (
    echo.%HERE%| find /I "claude-code-sepidz" >nul && goto AFTER_HEAL_RELAUNCH
