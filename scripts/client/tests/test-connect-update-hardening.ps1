...
Assert ($win -match 'function Write-UpdateFileLog[\s\S]{0,900}FileShare\]::ReadWrite') 'win UPDATE day-log write uses FileShare.ReadWrite (session-safe)'
Assert ($win -notmatch 'Write-UpdateFileLog[\s\S]{0,400}AppendAllText\(\(Get-UpdateLogPath\)') 'win UPDATE day-log does not use AppendAllText (exclusive-lock fail)'
$bump = Get-Content (Join-Path $RepoRoot 'publish\bump-connect-version.ps1') -Raw
Assert ($bump -match 'force_min_version') 'bump-connect-version locks force_min_version when mode=force'
$repair = Get-Content (Join-Path $RepoRoot 'scripts\client\windows\connect-env-repair.ps1') -Raw
Assert ($repair -match 'CLAUDE_CONNECT_TEST_INSTALL_CURRENT_PATH') 'install-current path has test seam env override'
Assert ($repair -match 'Test-ConnectFileIsStaleShadow') 'VerSrcComplete rejects STALE-SHADOW ui/diagnostic'
Assert ($repair -match 'Never stamp a Ver that is not content-complete') 'Set-ConnectInstallCurrent refuses incomplete VerDir write'
