...
# C7: empty UI_SLOT must not become primary-looking 0
Assert ($ui -notmatch "if \(-not \`\$slot\) \{ \`\$slot = '0' \}") 'Win SCORECARD must not coerce empty slot to 0'
Assert ($ui -match 'slot -notmatch') 'Win SCORECARD digit-only slot check'
Assert ($ui -match "slot = '\?'") 'Win SCORECARD uses ? for non-digit slot'
Assert ($sh -notmatch 'slot=0 ver=') 'Mac SCORECARD must not hardcode slot=0'
Assert ($sh -match 'slot_sc') 'Mac SCORECARD uses slot_sc'
