...
grep -q 'ensure_laptop_reverse_ssh' "$MAC" || fail 'connect.sh must call ensure_laptop_reverse_ssh'
...
grep -q 'ExitOnForwardFailure=yes' "$MAC" || fail 'tunnel must use ExitOnForwardFailure'
