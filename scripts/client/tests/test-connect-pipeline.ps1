...
foreach ($rel in @('windows\connect.ps1')) {
    ...
    Assert ($src -match 'Write-SessionBox|H hygiene|Show-ConnectHygieneInteractive') "$rel has session hygiene hotkey"
    ...
    Assert ($src -notmatch '"g" \{[^}]*Configure-GitMode') "$rel removed git menu option g"
