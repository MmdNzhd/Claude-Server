...
# SSH latency rollup (parity with Windows Add-SshMsSample / Write-SshLatencyRollupIfDue).
_SSH_MS_SAMPLES=""
_SSH_MS_SAMPLE_START_UNIX=0
_LAST_SSH_ROLLUP_UNIX=0
_LAST_AGENT_PATH_UNIX=0
_LAST_AGENT_PATH_OK=""
_LAST_AGENT_PATH_CONF=""

_ssh_ms_sample_add() {
    # Fail-open: never break sshx.
    local ms="${1:-0}" n=0 s
    case "$ms" in ''|*[!0-9]*) ms=0 ;; esac
    if [ "${_SSH_MS_SAMPLE_START_UNIX:-0}" -le 0 ] 2>/dev/null; then
        _SSH_MS_SAMPLE_START_UNIX="$(date +%s 2>/dev/null || printf '0')"
    fi
    _SSH_MS_SAMPLES="${_SSH_MS_SAMPLES:+${_SSH_MS_SAMPLES} }${ms}"
    for s in $_SSH_MS_SAMPLES; do n=$((n + 1)); done
    while [ "$n" -gt 60 ]; do
        _SSH_MS_SAMPLES="${_SSH_MS_SAMPLES#* }"
        n=$((n - 1))
    done
    _ssh_rollup_if_due || true
}

_ssh_rollup_if_due() {
    # Emit at most once/60s when count>=30 or window>=60s. Fail-open.
    [ -n "${_SSH_MS_SAMPLES:-}" ] || return 0
    declare -F connect_log >/dev/null 2>&1 || return 0
    local now elapsed n=0 min=0 max=0 p50=0 p90=0 over2=0 over5=0 win=1
    local sorted="" s idx
    now="$(date +%s 2>/dev/null || printf '0')"
    [ "$now" != "0" ] || return 0
    if [ $(( now - ${_LAST_SSH_ROLLUP_UNIX:-0} )) -lt 60 ]; then return 0; fi
    elapsed=$(( now - ${_SSH_MS_SAMPLE_START_UNIX:-$now} ))
    for s in $_SSH_MS_SAMPLES; do n=$((n + 1)); done
    [ "$n" -ge 1 ] || return 0
    if [ "$n" -lt 30 ] && [ "$elapsed" -lt 60 ]; then return 0; fi
    _LAST_SSH_ROLLUP_UNIX="$now"
    # Sort numerically for percentiles (simple space-separated list).
    sorted="$(printf '%s\n' $_SSH_MS_SAMPLES | sort -n | tr '\n' ' ')"
    sorted="${sorted% }"
    min="$(printf '%s\n' $sorted | head -1)"
    max="$(printf '%s\n' $sorted | tail -1)"
    idx=$(( (n - 1) / 2 ))
    ...
    p90="$(printf '%s\n' $sorted | sed -n "$((idx + 1))p")"
    for s in $sorted; do
        [ "$s" -ge 2000 ] 2>/dev/null && over2=$((over2 + 1))
        [ "$s" -ge 5000 ] 2>/dev/null && over5=$((over5 + 1))
    done
    win="$elapsed"
    [ "$win" -ge 1 ] || win=1
    connect_log "SSH_ROLLUP window_s=${win} count=${n} min=${min} p50=${p50} p90=${p90} max=${max} over_2s=${over2} over_5s=${over5}" 'INFO'
    _SSH_MS_SAMPLES=""
    _SSH_MS_SAMPLE_START_UNIX="$now"
}

_agent_path_probe_if_due() {
    # Fail-open: probe server conf TUNNEL_PORT vs this UI session port (rate-limited 60s).
    # Dual-UI: session!=conf with listen_conf=1 is ok (primary_match=0).
    # WARN only conf_empty|conf_port_closed|probe_fail.
    local session_port now out probe_line conf_port conf_am listen_conf listen_session
    local primary_match=0 reason="" ok=1 remote_cmd
    session_port="$(printf '%s' "${PORT:-${TUNNEL_PORT:-}}" | tr -dc '0-9')"
    if [ -z "$session_port" ]; then
        if declare -F connect_log >/dev/null 2>&1; then
            connect_log 'AGENT_PATH skip reason=no_session_port' 'TRACE'
        fi
        return 0
    fi
    now="$(date +%s 2>/dev/null || printf '0')"
    ...
    if [ -z "$probe_line" ]; then
        _LAST_AGENT_PATH_OK=0
        _LAST_AGENT_PATH_CONF=""
        if declare -F connect_log >/dev/null 2>&1; then
            connect_log "AGENT_PATH bad reason=probe_fail session_port=${session_port} conf_port= conf_am= listen_conf= listen_session= primary_match=0" 'WARN'
        fi
        return 0
    fi
    conf_port="$(printf '%s' "$probe_line" | sed -n 's/.*conf_port=\([^ ]*\).*/\1/p')"
    conf_am="$(printf '%s' "$probe_line" | sed -n 's/.*conf_am=\([^ ]*\).*/\1/p')"
    listen_conf="$(printf '%s' "$probe_line" | sed -n 's/.*listen_conf=\([^ ]*\).*/\1/p')"
    listen_session="$(printf '%s' "$probe_line" | sed -n 's/.*listen_session=\([^ ]*\).*/\1/p')"
    [ -n "$conf_port" ] && [ "$conf_port" = "$session_port" ] && primary_match=1
    if [ -z "$conf_port" ]; then
        ok=0
        reason='conf_empty'
    ...
    if declare -F connect_log >/dev/null 2>&1; then
        if [ "$ok" -eq 1 ]; then
            connect_log "AGENT_PATH ok session_port=${session_port} conf_port=${conf_port} conf_am=${conf_am} listen_conf=${listen_conf} listen_session=${listen_session} primary_match=${primary_match}" 'INFO'
        else
            connect_log "AGENT_PATH bad reason=${reason} session_port=${session_port} conf_port=${conf_port} conf_am=${conf_am} listen_conf=${listen_conf} listen_session=${listen_session} primary_match=${primary_match}" 'WARN'
        fi
    fi
}
...
sshx() {
    ...
    # Base64-wrap so nested quotes survive Mac ssh.
    # Old: bash -lc '' broke on any single quote (grep -E '^X=', ssh-keygen -N '', etc.).
...
# CONNECT_LOG_EARLY_FLUSH: flush early session log before UI draws.
...
while [ "$exit_requested" -eq 0 ]; do
    ...
    while [ -z "$go_path" ] && [ "$exit_requested" -eq 0 ]; do
        # Empty / no-visible: stay on a menu (a/u/q). Do not auto-jump into Add.
        if [ -z "$mounts_raw" ] || { [ -z "$mounts_visible" ] && [ "$hidden_count" -gt 0 ]; }; then
            ...
            printf '    \033[0;90mFirst project may need Remote Login / laptop SSH ready.\033[0m\n\n'
            if declare -F connect_prompt >/dev/null 2>&1; then choice="$(connect_prompt "    > " "MENU_PROJECT_EMPTY")"
            else read -rp "    > " choice; fi
            ...
            case "$choice" in
                a)
                    declare -F connect_log >/dev/null 2>&1 && connect_log 'EMPTY_MENU: add_after_prompt' 'INFO'
                    do_add
                    ...
                    ;;
                u)
                    if declare -F invoke_connect_manual_update >/dev/null 2>&1; then
                        invoke_connect_manual_update || true
                    else
    ...
    while [ "$main_continue" -eq 1 ]; do
        ...
        if declare -F log_session_context >/dev/null 2>&1; then log_session_context 'session_loop'; fi
            ...
            # Win Test-TunnelUp parity (TCP listen check).
            if ! tunnel_up; then
            ...
            if [ "${POST_TUNNEL_RECOVERY:-0}" = "1" ]; then
                if [ "$_on_folder" -eq 1 ]; then
                    declare -F connect_log >/dev/null 2>&1 && connect_log 'RECOVERY: skip_press_o_warn reason=already_on_folder' 'DEBUG'
                else
            ...
            while true; do
                ...
                # AGENT_PATH + SSH_ROLLUP host: internal 60s gates; fail-open.
                _agent_path_probe_if_due || true
                _ssh_rollup_if_due || true
                if read -r -t 1 -n 1 _key </dev/tty 2>/dev/null; then
                    ...
                    # Non-ASCII printable (e.g. Persian letters in window titles).
                    if [ -z "$_resolved" ]; then
