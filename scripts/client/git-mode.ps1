...
function Get-LastProjectId {
    $f = [System.IO.Path]::Combine($CfgDir, 'last.conf')
    if (-not (Test-Path $f)) { return '' }
    foreach ($ln in (Get-Content $f -ErrorAction SilentlyContinue)) {
        if ($ln -match '^LAST_PROJECT=(.+)$') { return $matches[1].Trim() }
    }
    return ''
}

function Save-LastProjectId {
    param([Parameter(Mandatory)][string]$ProjectId)
    Set-Content -Path ([System.IO.Path]::Combine($CfgDir, 'last.conf')) -Value "LAST_PROJECT=$ProjectId" -Encoding ASCII | Out-Null
}
