...
description: >-
  ...
  Use before the first mounts tool call — not after deny. Never remove or
...
On **Windows**, optional **windows-mcp** (Cursor MCP) can accelerate FS/UI — it **complements**
`laptop-exec`, never replaces it. **Mac / no MCP → laptop-exec only.**
...
```bash
laptop-exec status    # must be UP; else user connect.bat/sh — stop
...
## Hybrid: windows-mcp (Windows only — keep laptop-exec)
...
3. Prefer MCP process in **interactive session** (SessionId > 0). If Screenshot/UI
   fails with session/desktop errors → fall back to laptop-exec; tell user to run
...
|------|--------|-------------------|
| `git` (status, diff, commit, …) | — | **`laptop-exec git` only** |
...
- Forward: `windows-mcp-forward` → `http://127.0.0.1:18000/mcp` (Bearer from
  `~/.config/windows-mcp/env` / laptop `~\.windows-mcp\auth.key`).
...
Max 8 shared SSH slots; prefer ≤4 parallel laptop-exec; queue OK.
...
Prefer ≤4 parallel Tasks that touch the laptop. One hung `run` blocks everyone. No raw `ssh`/`scp`.
...
| "active_mount is already right" | Often `review` while workspace differs — always `-p` |
| "8 parallel explores are faster" | 8 slots → `session slots full` → everyone stalls |
| "I'll retry the denied tool" | Forbidden; run `NEXT:` |
| "windows-mcp replaces laptop-exec" | False — Mac/git/rg need LE; hybrid only |
| "MCP FileSystem search = rg" | False — use `laptop-exec rg` for content search |

## Red flags → stop

Retry after deny · `rg -i/--glob/-l` · Task without paste block · omit `-p` · `cd mounts && git` · long unbounded `run` · ask for SSHFS/sudo password → `sudo-from-laptop` · delete/disable laptop-exec because MCP exists
