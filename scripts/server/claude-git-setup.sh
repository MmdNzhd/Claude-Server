...
_tunnel_banner_matches_laptop() {
    local banner="${1:-}"
    [ -n "$banner" ] || banner="$(timeout 3 nc 127.0.0.1 "${TUNNEL_PORT:-0}" 2>/dev/null | head -1 | tr -d '\r\n')"
    [ -n "$banner" ] || return 1
...
_tunnel_up() {
    timeout 3 bash -c "exec 3<>/dev/tcp/127.0.0.1/${TUNNEL_PORT:-0}" 2>/dev/null || return 1
