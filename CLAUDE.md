...
**No Persian text anywhere in the repo** (scripts, docs, skills, plans, READMEs). English only - comments, docs, variable names, error messages, skill triggers. Keyboard-layout notes may say "Persian/Arabic layout" in English prose.
...
| Artifact | Path |
|---|---|
| Laptop day log (durable) | Win: `%USERPROFILE%\.config\claude-connect\logs\connect-YYYYMMDD.log` s, Mac: `~/.config/claude-connect/logs/connect-YYYYMMDD.log` |
...
5. Tunnel DOWN -> user `connect.bat`/`connect.sh`. STALE mount + UP tunnel -> MCP then laptop-exec (do not stall).
...
| `$script:ConnectVersion = '20260801.10'` | win connect.ps1 | Must match connect.bat guard + connect-version.txt |
| `CONNECT_VERSION='20260801.10'` | mac connect.sh | Must match published client version |
...
| Multi-agent s..."everyone blocked" | Ensure project `hooks.json` empty; user hooks use wrap; no Shell in `preToolUse`; slots/mux healthy; Task prompts force laptop-exec |
