...
**No Persian text anywhere in the repo** (scripts, docs, skills, plans, READMEs). English only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â comments, docs, variable names, error messages, skill triggers. Keyboard-layout notes may say "Persian/Arabic layout" in English prose.

## Architecture Overview

SSH reverse tunnel: client laptop ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ server, port = `20000 + (server_UID - 1000) * 10 + slot` (10-port non-overlapping block per user, slot 0-9; see `Get-TunnelPortUserBase` / `tunnel_port_user_base`).
**Source of truth = laptop disk.** Optional SSHFS under `~/mounts/<ID>/` is for Cursor UI only and may be STALE or NOT_MOUNTED.

**Agents:** priority+failover â€” Read/Grep mountâ†’MCPâ†’LE (~16â€“32); Write MCPâ†’mountâ†’LE (~8â€“10); Glob MCP search/list; git `laptop-exec` only. If 1st path fails, use next same turn. Connect UI `sshx()` / `SshX()` open one-shot SSH (no mux). `laptop-exec` uses a shared SSH ControlMaster through the same reverse tunnel.

```
Laptop ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬SSHÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¶ Server (port 22) + reverse tunnel (20000+UID)
Cursor agent on server ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬laptop-exec (ControlMaster)ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¶ 127.0.0.1:TUNNEL_PORT ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¶ laptop disk
Optional: Server ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬SSHFSÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¶ Laptop (UI; may be stale ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never prefer over laptop-exec)
```
...
scripts/
  client/
    ...
    users/designer/               # Designer-only (noVNC, no editor) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â separate product
  ...
  server/
    claude-server                 # CLI dispatcher ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ /usr/local/bin/claude-server
    commands/
    ...
    hooks/                        # Claude Code hooks ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ /usr/local/bin/
    claude-wrapper.sh
    claude-limits.conf
    claude-automount.sh
    claude-auth-sync.sh             # OAuth ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ ~/.claude/settings.json + empty credentials.json
    claude-auth-lib.py              # OAuth audit log, API probe, deploy helpers
    claude-auth-probe.sh            # Cron/manual token probe ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ /var/log/claude-auth.log
    cursor-auth-export.sh           # Export golden Cursor identity ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ /etc/cursor-auth/golden/
    cursor-auth-sync.sh             # Golden identity ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ ~/.config/Cursor/ per user
    ...
    laptop-exec.sh                # SSH-first CLI ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ /usr/local/bin/laptop-exec + per-user ~/.local/bin
...
docs/
  ...
  claude-design.md                # connect-design (claude.ai/design) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â separate from users/designer
publish/
...
sudo claude-server sync-auth      # After token change ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â pushes OAuth to all ~/.claude/
...
`add-user`, `claude-automount`, `sync-cursor-auth`, and cron (`cursor-auth-refresh` every 6h) keep `~/.config/Cursor/User/globalStorage/` aligned. Laptops only need `cursor.exe` + Remote-SSH ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â **no per-laptop Cursor login**.

**Chat uses laptop globalStorage ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â via an isolated profile:** Remote SSH Chat reads the *local* Cursor globalStorage on Windows, not the server's `.cursor-server` storage. `connect.ps1` opens Cursor with `--user-data-dir <LOCALAPPDATA>\ClaudeServerCursorProfile` (`Get-CursorRemoteProfileDir` in `editor-launch.ps1`) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a dedicated profile, separate from the developer's default `%APPDATA%\Cursor` profile. `cursor-auth-sync --force` runs on the server, then `cursor-auth-laptop.ps1` **merges** auth keys into that isolated profile's `state.vscdb` via SQLite UPSERT (never replaces the whole file, never closes any Cursor window). Many server-profile windows share one profile dir ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â one merge updates all of them. Personal Cursor (`%APPDATA%\Cursor`) is **never read, overwritten, or closed**.
...
| Server | Notes |
|---|---|
| `figma` | `https://mcp.figma.com/mcp` + golden OAuth `Authorization: Bearer figu_ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦` from `/etc/claude-code/figma-mcp.env` (not PAT `figd_`) |
| `context7` | `https://mcp.context7.com/mcp` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no API key required |
...
|---|---|---|
| `codebase-memory-mcp` | Code knowledge graph ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fewer tool calls, cheaper sessions (158-language tree-sitter + Hybrid LSP; replaced `codegraph` 2026-07-23, see `docs/superpowers/plans/2026-07-23-codebase-memory-mcp-rollout.md`) | per-user `install.sh` in `add-user.sh` (step 4b) |
| `headroom` | Context compression ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â reduces tokens sent to LLM | `pip3 install headroom-ai[mcp]` (step 10) |
| `sqlserver` | SQL Server query access via MCP | `npm install -g @bilims/mcp-sqlserver` |

**SQL Server connection settings** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â golden secrets live root-only in `/etc/claude-code/sqlserver.env` (`SQLSERVER_HOST` / `SQLSERVER_USER` / `SQLSERVER_PASSWORD`, mode `0600`). `cursor-mcp-sync` injects them into each user's Cursor `~/.cursor/mcp.json` and Claude `~/.claude/settings.json`. Optional per-user override: `~/.config/cursor-mcp/sqlserver.env`. **Never** commit passwords or bake them into git templates.
...
**codebase-memory-mcp indexing** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not auto-triggered on login (unlike the old codegraph). Manual trigger inside a project:
```bash
...
|---|---|---|
| `superpowers@claude-plugins-official` | Skills system ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â structured workflows, TDD, debugging, planning | official marketplace |
| `ecc@ecc` | Everything Claude Code ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â 261 skills, 64 agents, 84 commands | github: affaan-m/ECC |

The `settings.json` template lives in `add-user.sh` step 4 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â if you add/remove MCP servers or plugins, update that template.

**Gotcha:** `add-user.sh` must NOT use `chown -R` on `~/home/$user` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â SSHFS mounts under `~/mounts/` are owned by the remote user and will fail with `Operation not permitted`, aborting the script. Always chown specific subdirs only (`.claude/`, `.config/Cursor/`, `.local/bin/`, `.ssh/`).
...
| Artifact | Path |
|---|---|
| Laptop day log (durable) | Win: `%USERPROFILE%\.config\claude-connect\logs\connect-YYYYMMDD.log` Ãƒâ€šÃ‚Â· Mac: `~/.config/claude-connect/logs/connect-YYYYMMDD.log` |
...
[Cursor agent on Linux]
  Read/Grep: mount â†’ windows-mcp â†’ laptop-exec (~16-32 / ~8-12 / â‰¤4)
  WRITE/EDIT: MCP FileSystem â†’ mount â†’ LE (~8-10 / ~10 / â‰¤4)
  Glob: MCP search/list â†’ mount; git: laptop-exec only | Task paste PRIORITY+FAILOVER
       |
[mount / windows-mcp / laptop-exec] â†’ laptop disk
...
One MCP hard fail (`ECONNREFUSED` / not listed) â†’ MCP down for session; continue mount + LE.

Session: `~/.claude-connect.conf`. Cache: `~/.cache/laptop-exec/` (8 `slot-*.lock`, `cm-%C`).

### Hard rules (summary)

1. Hooks **ALLOW** Read/Grep/Glob/Write/Edit/EditNotebook/StrReplace/Delete/Shell on `/mounts/`. Read/Grep prefer mount (~16â€“32 parallel) then MCP; WRITE/EDIT prefer MCP FileSystem (~8â€“10) then mount (~10); Glob prefer MCP search/list (~8â€“12). Failover same turn if preferred path is down.
2. First I/O: table in skill + rule `laptop-exec` (measured 2026-07-26). **git** always `laptop-exec git`. Content Grep via MCP is Select-String (FileSystem search â‰  content).
3. `rg` is **not** ripgrep: `-i`/`-l`/`-n`/`--glob` rejected (old hangs pinned mux slots for hours).
4. Mux: **8** slots; prefer â‰¤4 LE parallel; mount/MCP fan-out to caps above; `session slots full` â†’ wait; no raw SSH storms.
5. Tunnel DOWN ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ user `connect.bat`/`connect.sh`. STALE mount + UP tunnel ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ MCP then laptop-exec (do not stall).
6. Sudo: `sudo-from-laptop --smart|--sepidz` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never ask for a password.
...
| `scripts/server/claude-automount.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `claude-automount` + cron unchanged |
| `scripts/server/claude-auth-sync.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `install -m 755 ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ /usr/local/bin/claude-auth-sync` |
| `scripts/server/claude-auth-lib.py` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `/usr/local/lib/claude-server/claude-auth-lib.py` |
| `scripts/server/claude-auth-probe.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `claude-auth-probe` + `/etc/cron.d/claude-auth-probe` |
| `scripts/server/cursor-auth-export.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `cursor-auth-export` + golden dir |
| `scripts/server/cursor-auth-sync.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `cursor-auth-sync` |
| `scripts/server/cursor-auth-refresh.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `cursor-auth-refresh` + cron |
| `scripts/server/cursor-auth-lib.py` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `/usr/local/lib/claude-server/cursor-auth-lib.py` |
| `scripts/server/cursor-remote-proxy-sync.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `cursor-remote-proxy-sync` + `/usr/local/lib/claude-server/` |
| `scripts/server/cursor-auth-source-path.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `cursor-auth-source-path` |
| `scripts/server/audit-cursor-golden-deep.py` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `/usr/local/lib/claude-server/audit-cursor-golden-deep.py` |
| `scripts/server/laptop-exec.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `laptop-exec` + skill + `cursor-rules/laptop-exec.mdc` + `cursor-hooks/` |
| `scripts/server/cursor-hooks/*` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â guard, wrap, shell-scan, session, hooks-user/project (project empty) |
| `scripts/server/claude-connect-logs-cleanup.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `claude-connect-logs-cleanup` + `/etc/cron.d/claude-connect-logs` |
| `scripts/server/sudo-from-laptop.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `sudo-from-laptop` (Smart/Sepidz sudo via laptop `*.local.ps1`) |
| `scripts/server/laptop-exec-setup.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `laptop-exec-setup` |
| `scripts/server/claude-mount.sh` | pushed on connect; also deploy via install.sh ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ `/usr/local/lib/claude-mount` |
...
| `scripts/server/commands/deploy-mount-fix.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `claude-server deploy-mount-fix` |
| `scripts/server/commands/deploy-client-bundle.sh` | deploy scripts ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `claude-server deploy-client-bundle` |
| `scripts/server/commands/*.sh` | install copies all to `/usr/local/lib/claude-server/` |

## Client Script Invariants

**Never break these ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â they are load-bearing:**

| Invariant | Location | Why |
|---|---|---|
| `PORT = 20000 + (UID-1000)*10 + slot` | `Get-TunnelPortUserBase` (git-mode.ps1), `tunnel_port_user_base` (git-mode.sh) | Non-overlapping 10-port block per user (fixed 2026-07-21 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â old `20000+UID+slot` overlapped up to 6/10 ports between adjacent UIDs); guard: `20000 < PORT ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¤ 65535` |
| `CM='$HOME/.local/bin/claude-mount'` | mac:12, win:28 | Single-quoted ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `$HOME` must expand on the REMOTE shell |
...
| `_tunnel_alive()` ps state check | mac:590 | `kill -0` returns 0 for zombie processes ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â must check `ps -o state=` and filter `Z` |
...
| `$script:ConnectVersion = '20260728.01'` | win connect.ps1 | Must match connect.bat guard + connect-version.txt |
| `CONNECT_VERSION='20260728.01'` | mac connect.sh | Must match published client version |
...
| Instant launcher `Claude-Connect.cmd` = direct `wscript //B //Nologo â€¦vbs` | connect-update / publish | No `start`, no `/MIN` |
...
| `CONNECT_PORT_BASE=20000` | mac connect.sh | Port formula base; guard: `base < PORT ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¤ 65535` |
...
| Invariant | Location | Why |
|---|---|---|
| EXIT + SIGTERM + **SIGHUP** traps | designer/connect.sh | Terminal close sends SIGHUP ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â without it, server mount is left dangling |
...
| `$novncOpened = $true` inside success branch only | designer/connect.ps1 | Same ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â do NOT set in the else/fail branch |
...
| Conf validated after source: `LAPTOP_USER` + `LAPTOP_PATH` non-empty | both | `set -uo pipefail` / PowerShell dies with cryptic error on unset vars ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â explicit die with clear message |
| `$SshDir` ACL grants both `$env:USERNAME` AND `$LaptopUser` when they differ | designer/connect.ps1 | Windows sshd reads `authorized_keys` under `$LaptopUser` token ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â directory must be listable by that user |
| `-L "127.0.0.1:${NOVNC_PORT}:127.0.0.1:${NOVNC_PORT}"` in tunnel | both | noVNC websockify binds `127.0.0.1` only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â must forward via SSH local port, not direct LAN |

## macOS SSH Detection (Three Layers)

`pgrep -x sshd` is unreliable on macOS with on-demand launchd SSH. Use this order:

1. `nc -zw1 127.0.0.1 22` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fastest
2. `launchctl print system/com.openssh.sshd | grep -q 'state = running'`
3. `launchctl list com.openssh.sshd >/dev/null 2>&1` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â exit code only (grep on output is a false positive)
4. `systemsetup -getremotelogin` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â slow fallback, requires sudo on newer macOS

## Client Codebase (Smart + Sepidz)

**One codebase** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `windows/connect.ps1` + `mac/connect.sh`. Same alias (`claude-server`), cfg (`~/.config/claude-connect`), port base (`20000 + (UID-1000)*10`, 10-slot non-overlapping block per user).
...
**Designer** (`users/designer/`) is a separate product (noVNC, no editor) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â also gets IP patch in the Sepidz ZIP only.
...
| Git hide fails (Cursor lock) | Retry rename 3ÃƒÆ’Ã¢â‚¬â€; stop git.exe on attempt 2; warn user; `G` remount |
...
| Multi-agent ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œeveryone blockedÃƒÂ¢Ã¢â€šÂ¬Ã‚Â | Ensure project `hooks.json` empty; user hooks use wrap; no Shell in `preToolUse`; slots/mux healthy; Task prompts force laptop-exec |
...
| Instant `Claude-Connect.cmd` = direct `wscript //B //Nologo â€¦vbs` | Wrap wscript in `start` or pass `-WindowStyle Hidden` to wscript.exe |
...
| `test-harder-live-bat-boot-handoff.ps1` | Bat â†’ boot handoff / versioned layout |
...
- Deploys Smart auto-update bundle to `smart@192.168.210.240` â†’ `/usr/local/share/claude-client` (unless skipped).
...
|---|---|---|
| 1 (preferred) | `Desktop\claude-publish\claude-code-client.zip` | Extract `windows\` â†’ `Desktop\Claude-Connect\`; run `Claude-Connect.vbs` or `connect.bat` |
...
**Gotcha:** Do NOT edit Chrome Preferences directly ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Chrome overwrites them on close.
...
These were deleted from the repo ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â use `claude-server` instead:
