...
After auth sync, if Chat messages fail or Cursor asks to log in: **Developer â†’ Reload Window** in the `[Claude Server]` profile window.
...
| Laptop Mac (temp) | `/tmp/claude-connect.*.log` via `mktemp` â€” deleted after flush |
| Laptop Windows (temp) | `%TEMP%\claude-connect-<PID>.log` â€” deleted on `Close-ConnectLog` |
...
- Client: `find â€¦ -mtime +1 -delete` on each sync flush
...
| No `connect.log` beside bat | Expected â€” logs are on the server only (v20260717.24+) |
