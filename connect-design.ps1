...
$TunnelPidFile = Join-Path $env:TEMP 'claude-design-tunnel.pid'
...
function Repair-SshPerm([string]$path) {
    ...
    # Do NOT remove SYSTEM — ssh-agent runs as SYSTEM and needs access
    icacls $path /remove:g "BUILTIN\Administrators" 2>$null | Out-Null
    icacls $path /grant:r "${LaptopUser}:F" 2>$null | Out-Null
}

function KillTunnels {
    # Also kill any PID we saved to file
    if (Test-Path $TunnelPidFile) {
        $savedPid = Get-Content $TunnelPidFile -ErrorAction SilentlyContinue
        if ($savedPid -match '^\d+$') {
    ...
    try {
        ...
        # CIM unavailable — best-effort: kill all ssh.exe owned by this session
...
if (-not $novncPort -or $novncPort -notmatch '^\d+$') {
    Die "Could not read valid noVNC port from server (got: '$novncPort')."
}
...
try {
    while ($keepRunning) {
        ...
        # Save PID for cleanup on unexpected exit
        $bgTunnel.Id | Set-Content $TunnelPidFile
        ...
        for ($i = 1; $i -le 15; $i++) {
            ...
            if ($bgTunnel.HasExited) {
                Write-Host " SSH exited (code $($bgTunnel.ExitCode))" -ForegroundColor Red
                if (Test-LocalPort $LocalPort) {
                    Write-Host "    Port $LocalPort is occupied by another process" -ForegroundColor DarkGray
                }
                break
            }
            if (Test-LocalPort $LocalPort) {
        ...
        if (-not $browserOpened) {
            ...
            $amp    = [char]38
            $url    = "http://localhost:${LocalPort}/vnc.html?autoconnect=true${amp}quality=9${amp}compression=0${amp}reconnect=true${amp}reconnect_delay=2000${amp}view_only=false"
            $chrome = "C:\Program Files\Google\Chrome\Application\chrome.exe"
            $edge   = "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"
            if (Test-Path $chrome) {
                Start-Process $chrome -ArgumentList @('--kiosk', $url)
            } elseif (Test-Path $edge) {
                Start-Process $edge -ArgumentList @('--kiosk', '--kiosk-type=fullscreen', $url)
            } else {
                Start-Process $url
            }
            StepOk $url
        ...
        if ($bgTunnel.HasExited -and -not ($gotKey -and $action -eq 'q')) {
            $kickRaw = & ssh -n -o ClearAllForwardings=yes -o BatchMode=yes -o ConnectTimeout=10 `
                $Alias "designer-start check-kicked 2>&1"
            $kickSshOk = ($LASTEXITCODE -eq 0)
            $kickCheck = ($kickRaw) -join ""

            if (-not $kickSshOk -or $kickCheck -notmatch 'OK|KICKED') {
                # SSH failed — cannot verify kick status
                Write-Host ""
                Write-Host "    Connection lost (could not verify kick status)" -ForegroundColor Yellow
                Write-Host "    R = try reconnect   Q = exit" -ForegroundColor DarkGray
                $rk = ''
                while ($rk -ne 'r' -and $rk -ne 'q') {
                    $ki3 = SafeReadKey
                    if ($ki3) {
        ...
        if ($sessionOut -match 'OK') {
            $np = ($sessionOut -split "`n" |
                Where-Object { $_ -match '^NOVNC_PORT=' } |
                Select-Object -First 1) -replace '^NOVNC_PORT=',''
            if ($np -and $np -match '^\d+$') {
                $novncPort = $np
            } else {
                Warn "Reconnect: invalid or missing port '$np'; reusing port $novncPort"
            }
        } else {
